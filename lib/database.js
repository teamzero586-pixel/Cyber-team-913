...
// Wipes a number's saved brand override so it falls back to config.js
// defaults again (BOT_NAME / OWNER_NUMBER / etc). Needed because a brand
// record, once saved, takes priority over config.js forever — updating
// config.js alone won't change what .owner/.menu show for a number that
// already has one on file.
async function deleteBotBrand(number) {
    try {
        const cleanNumber = number.replace(/[^0-9]/g, '');
        await BotBrand.deleteOne({ number: cleanNumber });
        console.log(`🎨 Brand override cleared for ${cleanNumber} — will use config.js defaults now`);
        return true;
    } catch (error) {
        console.error('❌ Error deleting brand settings:', error.message);
        return false;
    }
}
